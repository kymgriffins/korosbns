...
export default function AdminProfilePage() {
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      const [me, links] = await Promise.all([
        ...
        adminProfileApi.getSocialLinks(),
      ]);
      setProfile(me);
  ...
  async function handleSave() {
    ...
    try {
      if (avatarFile) {
        await adminProfileApi.patchAvatar(avatarFile);
      }
      await adminProfileApi.patchMe({
  ...
  async function handleAddLink() {
    ...
    setAddingLink(true);
    try {
      await adminProfileApi.upsertSocialLink({
      ...
      setSocialLinks(await adminProfileApi.getSocialLinks());
    } catch (err) {
  ...
  async function handleDeleteLink(platform: string) {
    try {
      await adminProfileApi.deleteSocialLink(platform);
            ...
            <div className="flex items-center gap-4">
              <div className="group relative">
                <Avatar className="size-16 ring-2 ring-border">
                  <AvatarImage src={avatarPreview || profile.avatar_url || profile.avatar || ""} />
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Select
                  ...
                  onValueChange={(v) => setProfile({ ...profile, profile_visibility: v })}
                >
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            {socialLinks.length === 0 && (
              <p className="py-6 text-center text-sm text-muted-foreground">No social links added yet</p>
            )}
              ...
              return (
                <div key={link.platform} className="flex items-center gap-3 rounded-lg border p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium capitalize">{link.platform}</p>
            ...
            <div className="space-y-3">
              <p className="text-xs font-medium tracking-wider text-muted-foreground uppercase">Add Link</p>
