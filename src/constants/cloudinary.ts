/** @deprecated Prefer `@/content` media JSON — compatibility shim. */
import { mediaContent } from "@/content";

export const CLOUDINARY_HERO_BARAZA = mediaContent.cloudinary.heroBaraza;
export const CLOUDINARY_HERO_LANDING_VIDEO_MP4 = mediaContent.cloudinary.heroLandingVideoMp4;
export const LANDING_YOUTUBE_EMBED = mediaContent.cloudinary.youtubeEmbed;
export const CLOUDINARY_COUNTY_BUDGET_SOCIAL_VIDEO =
  mediaContent.cloudinary.countyBudgetSocialVideo;
export const CLOUDINARY_PARTNERSHIPS = mediaContent.partnerships;
