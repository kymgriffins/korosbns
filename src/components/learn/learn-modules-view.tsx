        ...
        <div className="flex items-center gap-2 shrink-0">
          <div className="relative hidden sm:block">
            ...
            <input
              ...
              className="pl-8 pr-3 py-1.5 bg-muted/40 border-0 rounded-lg text-xs w-36 focus:outline-none focus:ring-2 focus:ring-primary/20 transition-all"
            />
          </div>
          {profile?.avatar_url ? (
        ...
        <div className="max-w-6xl mx-auto space-y-3">
          ...
          {filteredModules.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-2.5">
              {filteredModules.map(({ stage, completedCount, total, isCompleted, isInProgress }, idx) => (
                <motion.div
                  ...
                  className="group bg-card shadow-xs hover:shadow-sm rounded-xl p-3.5 cursor-pointer hover:bg-accent/30 transition-all flex flex-col"
                  ...
                  <div className="flex items-center justify-between pt-2 border-t border-border/30 mt-auto">
                    <div className="flex items-center gap-1.5 min-w-0">
                      {stage.author ? (
                        <Link href={Routes.LearnAuthor(getAuthorSlug(stage.author))} className="flex items-center gap-1.5 min-w-0 group" onClick={(e) => e.stopPropagation()}>
                          {stage.author.image ? (
                            ...
                            <div className="size-[18px] rounded-full bg-muted flex items-center justify-center shrink-0">
                              <span className="text-[7px] font-bold text-muted-foreground">{stage.author.name[0]}</span>
                            </div>
                          )}
                          <span className="text-[9px] font-semibold text-muted-foreground group-hover:text-primary transition-colors truncate">
                            {stage.author.name}
                          </span>
                        ...
                        <div className="flex items-center gap-1.5 min-w-0">
                          <div className="size-[18px] rounded-full bg-muted flex items-center justify-center shrink-0">
                            <span className="text-[7px] font-bold text-muted-foreground">{(stage.credits || "BNS")[0]}</span>
                          </div>
            ...
            <div className="text-center py-16">
              <BookOpen className="size-10 text-muted-foreground/15 mx-auto mb-3" />
