...
export function LearnModulesView({ profile, stages, currentStage, onSelectStage, onRefresh }: LearnModulesViewProps) {
  ...
  return (
    <div className="flex flex-col h-full bg-background overflow-hidden">
      <header className="flex items-center justify-between px-4 md:px-5 py-3 border-b shrink-0 gap-3">
        <div className="flex items-center gap-3 min-w-0">
          <div className="flex size-9 items-center justify-center rounded-lg bg-primary/10">
            <BookOpen className="size-4 text-primary" />
          </div>
          <div className="min-w-0">
            <h1 className="font-bold text-base leading-tight truncate">Civic Modules</h1>
            <p className="text-sm text-muted-foreground">Master the budget process</p>
          </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          <div className="relative hidden sm:block">
            ...
            <input
              ...
              className="pl-8 pr-3 py-1.5 bg-muted/40 border-0 rounded-lg text-sm w-36 focus:outline-none focus:ring-2 focus:ring-ring/30 transition-all"
            />
          </div>
          {onRefresh && (
            <button
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-4">
        <div className="max-w-6xl mx-auto space-y-4">
          <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 scrollbar-hide">
            {[
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-sm font-semibold transition-all shrink-0 focus-visible:ring-2 focus-visible:ring-ring ${
                  activeTab === tab.key
          ...
          <div className="flex items-center gap-1.5 overflow-x-auto pb-0.5 scrollbar-hide border-t pt-3">
            {[
              { key: "all", label: "All" },
          ...
          {filteredModules.length > 0 ? (
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
              {filteredModules.map(({ stage, completedCount, total, isCompleted, isInProgress }, idx) => (
                  <motion.div
                    ...
                    className="group bg-card rounded-xl border p-3.5 cursor-pointer hover:border-primary/30 hover:shadow-sm transition-all flex flex-col"
                    onClick={() => router.push(`/learn/modules/${stage.slug}`)}
                  >
                  <HarmonizedImage
                    src={stage.image_url}
                    alt={stage.title}
                    className="-mx-0.5 -mt-0.5 mb-2.5 rounded-lg"
                  ...
                  <h3 className="font-bold text-sm leading-tight mb-1 group-hover:text-primary transition-colors">{stage.title}</h3>
                  <p className="text-sm text-muted-foreground line-clamp-2 mb-2 flex-1">{stage.description || ""}</p>
                  ...
                  <div className="flex items-center justify-between pt-2 border-t mt-auto">
                    <div className="flex items-center gap-1.5 min-w-0">
                      {stage.author ? (
                        <Link href={Routes.LearnAuthor(getAuthorSlug(stage.author))} className="flex items-center gap-1.5 min-w-0 group" onClick={(e) => e.stopPropagation()}>
                          {stage.author.image ? (
                            ...
                            <div className="size-[18px] rounded-full bg-muted flex items-center justify-center shrink-0">
                              <span className="text-[9px] font-bold text-muted-foreground">{stage.author.name[0]}</span>
                            </div>
                          )}
                        ...
                        <div className="flex items-center gap-1.5 min-w-0">
                          <div className="size-[18px] rounded-full bg-muted flex items-center justify-center shrink-0">
                            <span className="text-[9px] font-bold text-muted-foreground">{stage.credits[0]}</span>
                          </div>
                    ...
                    <Button size="sm" className="rounded-lg h-7 px-3 text-sm font-semibold shrink-0 focus-visible:ring-2 focus-visible:ring-ring"
            ...
            <div className="text-center py-16">
              ...
              <p className="text-base font-semibold text-muted-foreground">No modules found</p>
