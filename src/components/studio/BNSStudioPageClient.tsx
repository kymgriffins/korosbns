...
import { ArrowUpRight, Send, ShieldCheck, ChevronDown } from "lucide-react";
...
export function BNSStudioPageClient() {
  ...
  return (
    <div className="w-full bg-background text-foreground selection:bg-primary/30">
      ...
      <section className="relative border-t border-border/40 bg-card text-foreground px-6 py-20 md:py-28">
        <GsapReveal className="mx-auto max-w-6xl">
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-12 lg:items-center">
            <div className="lg:col-span-7 space-y-6">
              ...
              <h2 className="text-3xl font-black tracking-tight text-foreground md:text-5xl lg:leading-tight">
              ...
              <div className="grid grid-cols-3 gap-6 border-t border-border/50 pt-6">
                <div>
                  <p className="text-2xl font-bold text-foreground md:text-3xl">08</p>
                  <p className="text-xs text-muted-foreground mt-1">Production Formats</p>
                </div>
                <div>
                  <p className="text-2xl font-bold text-primary md:text-3xl">47</p>
                  <p className="text-xs text-muted-foreground mt-1">Counties Subsidized</p>
                </div>
                <div>
                  <p className="text-2xl font-bold text-foreground md:text-3xl">2×</p>
                  <p className="text-xs text-muted-foreground mt-1">Mission Multiplier</p>
                </div>
              </div>
            ...
            <div className="lg:col-span-5 space-y-4">
              <div className="rounded-3xl border border-border/60 bg-muted/30 p-8 space-y-6">
                <div className="flex items-center gap-3">
                  <div className="flex size-10 items-center justify-center rounded-xl bg-primary/10 text-primary">
                    <ShieldCheck className="size-5" />
                  </div>
                  <div>
                    <h3 className="font-heading text-base font-bold text-foreground">Sovereign Standard</h3>
                    <p className="text-xs text-muted-foreground">Certified non-partisan public auditing</p>
                  </div>
                </div>
                <p className="text-xs leading-relaxed text-muted-foreground">
                  BNS Studios operates under the exact same verification standard as the BNS Foundation. Every chart, statistic, and narrative passed to the public is vetted against National Treasury and Auditor-General records.
                </p>
                <div className="border-t border-border/40 pt-4 flex items-center justify-between text-xs font-mono">
                  <span className="text-muted-foreground">Audit Rigour:</span>
                  <span className="font-bold text-foreground">100% Primary Source</span>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* 03 — Production Formats Spectrum */}
      <section className="relative border-t border-border/40 bg-background text-foreground px-6 py-20 md:py-28">
        <div className="mx-auto max-w-6xl">
          ...
          <GsapStaggerReveal className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {STUDIO_CONTENT_TYPES.map((type) => {
              ...
              return (
                <div
                  key={type.id}
                  data-gsap-item
                  className="group flex flex-col justify-between rounded-2xl border border-border/60 bg-card p-6 transition-all duration-300 hover:border-primary/50 hover:bg-muted/30 hover:shadow-sm"
                >
                  <div>
                    <div className="flex size-10 items-center justify-center rounded-xl border border-border/60 bg-muted text-primary transition-colors group-hover:bg-primary group-hover:text-primary-foreground">
      ...
      <section className="border-t border-border/40 bg-muted/20 text-foreground px-6 py-20 md:py-28">
        <div className="mx-auto max-w-6xl">
          <GsapReveal className="flex items-center justify-between mb-12">
            <div>
              <p className="text-xs font-bold uppercase tracking-wider text-primary">Evidence</p>
              <h2 className="mt-2 text-3xl font-extrabold text-foreground md:text-4xl">Featured Commissions</h2>
            </div>
            <PillButtonGroup
              href="/bns-studio/work"
          ...
          <GsapStaggerReveal className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {featuredProjects.map((project) => (
              <Link
                key={project.id}
                data-gsap-item
                href={`/bns-studio/${project.slug}`}
                className="group relative block overflow-hidden rounded-3xl border border-border/60 bg-card transition-all duration-300 hover:border-primary/50 hover:shadow-md"
              >
      ...
      <section className="border-t border-border/40 bg-background px-6 py-20 md:py-28">
