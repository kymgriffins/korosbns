...
import type { CareersContent, PartnerPageSectionsContent } from "@/lib/cms-live-data";
import { isSectionVisible } from "@/lib/partner-page-cms";

const cultureIconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  Globe,
  Zap,
  Users,
  Award,
...
export type CareersLandingProps = {
  careersData?: CareersContent;
...
export function CareersLanding({
  ...
  const data = careersData ?? careersContent;
  const openRoles = data.openRoles as JobRole[];
  const culturePillars = data.culturePillars;

  const show = (sectionId: string) =>
    isSectionVisible("careers", sectionId, sectionsConfig);
  ...
  const filteredRoles = useMemo(() => {
    if (selectedDept === "All") return openRoles;
    return openRoles.filter((r) => r.department === selectedDept);
  }, [selectedDept, openRoles]);
      ...
      {show("hero") ? (
        <section
          className={cn(
            ...
            "relative overflow-hidden border-b border-border/60 bg-muted/20",
          )}
        >
          ...
          <div className={SECTION_SHELL_INNER}>
            <div className="max-w-3xl mx-auto text-center mb-12 lg:mb-16 space-y-3">
              ...
              <h2 className="text-3xl md:text-4xl font-bold font-heading text-foreground tracking-tight">
                {data.whyJoinSection.title}
              </h2>
              <p className="text-muted-foreground text-sm md:text-base leading-relaxed">
                {data.whyJoinSection.description}
              </p>
            </div>
        ...
        <section id="open-roles" className="py-16 lg:py-24">
          <div className={SECTION_SHELL_INNER}>
            <div className="max-w-3xl mx-auto text-center mb-10 space-y-3">
              ...
              <h2 className="text-3xl md:text-4xl font-bold font-heading text-foreground tracking-tight">
                {data.openRolesSection.title}
              </h2>
              <p className="text-muted-foreground text-sm md:text-base">
                {data.openRolesSection.description}
              </p>
            </div>
              ...
              {filteredRoles.map((role) => (
                ...
                >
                  ...
                  <div className="pt-6 mt-4 border-t border-border/40 flex items-center justify-between gap-3">
                    <Button asChild size="sm" className="w-full font-bold gap-1.5 text-xs">
                      <Link
            ...
            {filteredRoles.length === 0 && (
              <div className="text-center py-12 text-muted-foreground text-sm">
                No positions open in this department right now. Reach out with a general
                application below!
              </div>
            )}
          </div>
          ...
          <div className={SECTION_SHELL_INNER}>
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {data.process.steps.map((st) => (
                <div
                  key={st.step}
                  className="p-6 rounded-2xl border border-border/60 bg-card space-y-2"
                >
                  <span className="text-2xl font-bold font-mono text-primary/70">
                    {st.step}
                  </span>
