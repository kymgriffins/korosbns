...
>(
  (
    ...
    const isUnlocked =
      achievement.achievedAt !== null || (achievement.progress ?? 0) >= 100
    const progress =
      typeof achievement.progress === "number"
        ? Math.min(100, Math.max(0, achievement.progress))
        : 0
        ...
        >
          ...
          {achievement.badgeUrl ? (
            ...
            <div
              ...
              className={cn(
                badgeSizeMap[badgeSize],
                "relative z-10 flex items-center justify-center rounded-full text-2xl",
                isUnlocked ? "bg-muted/60" : "bg-muted/30 grayscale",
              )}
            >
              {achievement.iconEmoji}
            </div>
