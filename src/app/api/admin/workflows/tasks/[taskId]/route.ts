...
const endpoint = (taskId: string) => `${API_BASE}/api/admin/workflows/tasks/${taskId}/`;
...
export async function PATCH(
  ...
  return withRetry(endpoint(taskId), "PATCH", body);
}
...
export async function DELETE(
  ...
  const { taskId } = await params;
  return withRetry(endpoint(taskId), "DELETE");
}
