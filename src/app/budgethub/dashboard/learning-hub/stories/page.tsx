...
export default function StoriesPage() {
  ...
  const fetchData = useCallback(async () => {
    setLoading(true);
    try {
      const res = await contentData.stories.fetch();
    ...
    <div className="flex flex-col gap-4">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Budget stories and narratives</p>
        </div>
          ...
          {items.map((item) => (
            ...
            >
              {item.thumbnail_url ? (
                ...
                <div className="flex aspect-video w-full items-center justify-center bg-gradient-to-br from-amber-500/10 to-amber-500/5">
                  <BookOpen className="size-10 text-amber-400/40" />
                </div>
              )}
