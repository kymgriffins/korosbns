...
function isPersistedId(id: string) {
  return !id.startsWith("local-") && !id.startsWith("template-");
}
    ...
    >
      ...
      >
        ...
        {assigneeLabel ? (
          ...
          <span className="shrink-0 text-[10px] italic text-muted-foreground/60">Unassigned</span>
        )}
    ...
    try {
      if (isPersistedId(id)) {
        ...
        applyItemsChange((prev) =>
          mergeChecklistOrder(prev.map((i) => (i.id === id ? { ...updated, ...patch, id } : i))),
        );
      } else {
        ...
        applyItemsChange((prev) =>
          mergeChecklistOrder(prev.map((i) => (i.id === id ? { ...created, id: newId } : i))),
        );
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-[11px] text-muted-foreground">
            Title + assignee only · {active.length} open · {done.length} completed
          </p>
        </div>
        {!readonly && (
