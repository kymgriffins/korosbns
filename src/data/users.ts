...
export const userData: UserDataStore = {
  ...
  admin: {
    users: {
      ...
      /** Prefer invitations; legacy create form maps to invite when email+role present. */
      ...
      update: (id: string, data: Partial<AdminUser>) =>
        withFallback(
          "users",
          () => applyUserWrite(id, data),
          () => null,
        ),
      delete: (id: string) =>
        withFallback(
          "users",
          () => adminUsersApi.deactivate(id).then(() => true),
          () => true,
        ),
    },
    authors: {
      fetch: (_params?: { page?: number; search?: string }) =>
        withFallback(
          "users",
          () =>
      ...
      fetchBySlug: (slug: string) =>
        withFallback(
          "users",
          () => adminAuthorsApi.get(slug).then((r) => r.author),
          () => null,
        ),
      ...
      update: (_slug: string, _data: Partial<AdminAuthor>) =>
        withFallback(
          "users",
          async () => {
